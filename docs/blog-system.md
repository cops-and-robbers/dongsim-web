...
그래서 동기화 때 한 번 받아 **R2에 굽고 본문의 주소를 바꿔 끼운다** (요청 시점 프록시는
3단계에서 제거했다 - 발행 시점에 이미 최적화된 파일이 만들어져 있다).

- 키는 내용 해시 + 치수: `images/<해시>-<가로>x<세로>.webp` - 같은 그림은 재동기화해도 안 올라간다(멱등)
- sharp 리사이즈(긴 변 1600) + WebP 변환, GIF·SVG는 원본 그대로
- 파일 이름의 치수를 화면이 읽어 그림 자리를 미리 잡는다(CLS 방지)
- 참조가 끊긴 옛 파일은 어드민 → 블로그의 "이미지 저장소 정리"로 지운다 (7일 유예)
...
- **커버 비율은 목록·상세 모두 3:2**(`aspect-3/2` + `object-cover`)로 통일한다.
  목록에서 클릭한 썸네일 프레임이 상세 커버와 같게 보여 연속성이 생기고, 3:2가 16:9보다
  덜 잘라내 인물·중요 부분이 유지된다. 한쪽만 바꾸면 같은 사진이 다르게 잘려 어긋난다
