...
- 사이트 전역 값(팀원·링크·브랜드)은 `lib/constants.ts` 한 곳만 고친다
- **사용자에게 보이는 문구(카피)는 `lib/i18n/messages.ts`가 단일 출처.** 컴포넌트에 하드코딩하지 않고, 섹션은 `copy` prop으로 받는다. 공통 UI 문구는 `lib/i18n/chrome.ts`. 상세는 `docs/i18n.md`
- 한 글자 변수(`m`·`c` 등) 지양, 의미 있는 이름. 반복되는 구조(hreflang 언어 맵 등)는 헬퍼로 (`alternateLanguages`)
